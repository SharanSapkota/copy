...
router.get('/likedProducts', AuthController.authUser, async(req,res) =>{
    try{
        const likedPosts = await Profiles.findOne({user : req.user.id}, 'liked_items').populate('liked_items')
